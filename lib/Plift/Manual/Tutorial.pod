...
Before proceding, we must learn about the Plift::Context object. At a high level,
you can think of the context object as a representation of the template that we
are procesing. Thats why we create a new context via L<Plift/template>, and
render it via L<Plift::Context/render>. Which gives us a nice syntax:
...
duration of that template rendering (usualy a web request). In contrast to the
actual L<Plift> instance that is meant to live for the whole application life.

The methods L<Plift/render> and L<Plift/process> methods are just wrappers
around L<Plift::Context/set>, L<Plift::Context/at> and L<Plift::Context/render>.
The code snipped given above is equivalent to:
